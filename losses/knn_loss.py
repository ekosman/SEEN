    ...
    def forward(self, x):
        ...
        for x_i, (x_, x_neighbors_indices) in enumerate(zip(x, i)):
            ...
            if self.iteration % 5000 == 0:
                ...
                plt.figure()
                ax1.set_title(f"fdsfsd {self.iteration}")
                ax1.plot(diff_, label='distances')
                ax1.plot([0, len(diff_) - 1], [diff_[0], diff_[-1]], color='r')
                ax1.plot(ds, label='difference')
                ax1.axvline(chosen, color='g', label=f"chosen threshold: {diff_[chosen]} @ {chosen}")
                ax1.legend()
                ...
                # plt.figure()
                ax2.set_title(f"Iteration {self.iteration}, max distance: {diff.max()}")
                ax2.hist(diff_, bins=50)
